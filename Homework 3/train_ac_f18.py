...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    """
    A = tf.layers.dense(inputs=input_placeholder, units=size, activation = activation)
    layers = [A]


    #We append however many layers we need to
    for i in range(n_layers-1):
        layers.append( tf.layers.dense(inputs=layers[i] , units=size, activation = activation) )

    #We define the output layer
    output_placeholder = tf.layers.dense(inputs=layers[-1], units=output_size, activation = output_activation)
...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        #Advantages are just real numbers
        #Chris
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        """
        if self.discrete:
            #Okay, here I am treating logit as just raw returns
            #Chris
            sy_logits_na= build_mlp(input_placeholder = sy_ob_no, output_size = self.ac_dim , scope =None , n_layers = self.n_layers, size = self.size, output_activation = None)
            return sy_logits_na

        else:
            sy_mean = build_mlp(input_placeholder = sy_ob_no, output_size = self.ac_dim , scope =None , n_layers = self.n_layers, size = self.size, output_activation = None)
            #I do not know what to do with logstd.
            #Chris
            sy_logstd = tf.Variable(tf.zeros([self.ac_dim]), name="logstd", dtype=tf.float32)
    ...
    def sample_action(self, policy_parameters):
        ...
        """
        if self.discrete:
            #We define a probability distribution and sample
            sy_logits_na = policy_parameters
            dist = tf.distributions.Categorical(logits=sy_logits_na)
            sy_sampled_ac = dist.sample(name = 'sample')
        else:
            sy_mean, sy_logstd = policy_parameters
            #Chris
            #We sample from the action space
            std =tf.exp(sy_logstd)
            sy_sampled_ac = tf.add(tf.multiply(tf.random_normal(shape= tf.shape(sy_mean)) ,std ), sy_mean)
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # I tried my best to vectorize, I don't think it worked.
            # Right now I have it returning a list, which is what I think is desired?
            sy_logprob_n = -tf.nn.sparse_softmax_cross_entropy_with_logits(labels= sy_ac_na, logits= sy_logits_na)

#            sy_logprob_n =   tf.log(tf.boolean_mask( sy_logits_na  ,  tf.one_hot(sy_ac_na, self.ac_dim)  )  )
        else:
            sy_mean, sy_logstd = policy_parameters
            # We define a multivariate normal distribution
            # mvn = tf.contrib.distributions.MultivariateNormalDiag(
            # loc=sy_mean,
            # scale_diag=tf.exp(sy_logstd))

            sy_z = tf.divide(tf.subtract(self.sy_ac_na, sy_mean), tf.exp(sy_logstd))
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        q_n = self.sess.run(re_n + (1- terminal_n) * self.gamma * self.critic_prediction, feed_dict = {self.sy_ob_no:next_ob_no} )
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            target_values = re_n + (1-terminal_n)*self.sess.run(self.critic_prediction, feed_dict = {self.sy_ob_no: next_ob_no} )
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        q_n, adv_n = agent.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n)
        ...
        if itr == n_iter -1:
            saver.save(agent.sess, '~/my_test_model')
#            tf.saved_model.simple_save(agent.sess, '~/model2.ckpt', inputs = {"inputs": agent.sy_ob_no},  outputs = {"outputs": agent.sy_sampled_ac})
