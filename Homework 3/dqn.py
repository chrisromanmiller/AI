...
class QLearner(object):
  ...
  def __init__(
    ...
    ######
    self.q_function = q_func(obs_t_float, self.num_actions, scope="q_func", reuse=False)
    target_q_function = q_func(obs_tp1_float, self.num_actions, scope="target_q_func", reuse=False)

    target = self.rew_t_ph + (1.0 -self.done_mask_ph)*gamma * tf.reduce_max( target_q_function, axis =1 )
    q_ret = tf.reduce_sum(self.q_function * tf.one_hot(self.act_t_ph, self.num_actions), axis =1 )  


    q_func_vars =tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = 'q_func')
    target_q_func_vars =tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = 'target_q_func')

    self.total_error = huber_loss(tf.subtract( q_ret , target))
  ...
  def step_env(self):
    ...
    #1) Store last observation
    self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)
    processed_observation = self.replay_buffer.encode_recent_observation()


    #2) advance one step 
    epsilon = .1
    if (np.random.uniform() < 1. - epsilon) and self.model_initialized ==True:
      max_action = self.session.run(tf.argmax(self.q_function, axis = 1) , feed_dict = {self.obs_t_ph: [processed_observation]} ) 
      action = max_action
    else:
    ...
    #3)store outcome into replay buff
  ...
  def update_model(self):
    ...
    if (self.t > self.learning_starts and \
      ...
      #####
      obs_t_batch, act_batch, rew_batch, obs_tp1_batch, mask_batch =  self.replay_buffer.sample(self.batch_size)
